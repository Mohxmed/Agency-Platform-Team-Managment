  ...
  if (variant === "row") {
    return (
      ...
      >
        ...
        {/* Row 1: name + state + progress */}
        <div className="flex items-center justify-between gap-4">
          <Link
            href={`/dashboard/team/projects/${project.id}`}
            className="group/head flex min-w-0 items-center gap-3"
          >
            {/* Icon */}
            <div
        ...
        <div className="mt-3.5 flex flex-wrap items-center justify-end gap-x-4 gap-y-2 border-t border-dashed border-ink/[0.07] pt-3.5">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            <Link
              href={`/dashboard/team/projects/${project.id}`}
              className={`inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg px-3 text-[11px] font-black transition-all duration-200 hover:gap-2 ${theme.solid} ${theme.solidHover} ${theme.solidText}`}
            >
              عرض المشروع
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col p-5">
        ...
        <Link
          href={`/dashboard/team/projects/${project.id}`}
          className="group/head flex items-start justify-between gap-3"
        >
          ...
          <ChevronLeft className="h-4 w-4 shrink-0 text-ink/30 transition-all duration-200 group-hover/head:-translate-x-0.5 group-hover/head:text-primary dark:text-ink/40" />
        </Link>
        ...
        <div className="mt-4">
          <div className="mb-1.5 flex items-center justify-between text-[11px] font-bold">
            ...
            <span
              className={
                progress >= 100 ? "text-emerald-600 dark:text-emerald-400" : "text-ink"
              }
            >
              {progress}%
            </span>
        ...
        <div className="mt-4 flex items-center justify-between gap-3 border-t border-dashed border-ink/[0.07] pt-3.5">
          <div className="flex min-w-0 items-center gap-3">
            {members.length > 0 ? (
              <div className="flex items-center -space-x-2 rtl:space-x-reverse">
                {members.slice(0, MAX_VISIBLE_MEMBERS).map((member) => (
